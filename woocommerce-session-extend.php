<?php
/**
* Plugin Name: WooCommerce Session Extend
* Plugin URI: github.com
* Description: This plugin simply extends your cart sessions so users can keep their cart items for up to 30 days
* Version: 1.0
...
add_action('plugins_loaded', 'load_woocommerce_session_extend_class');

//if ( ! class_exists( 'WooCommerce' ) ) exit; // exit if woocommerce is not installed
function load_woocommerce_session_extend_class(){

	add_filter('woocommerce_session_handler', 'replace_with_custom_session_handler_class');


	

	function replace_with_custom_session_handler_class(){
	return 'WC_Custom_Session_Extend_Handler';
	}
 ...
 * @author 		WooThemes
 */
class WC_Custom_Session_Extend_Handler extends WC_Session {
    ...
    public function set_session_expiration() {
	    $this->_session_expiring    = time() + intval( apply_filters( 'wc_session_expiring', 60 * 60 * 719 ) ); // 30 Days
		$this->_session_expiration  = time() + intval( apply_filters( 'wc_session_expiration', 60 * 60 * 720 ) ); // 30 days Hours
    }
...
}// load_woocommerce_session_extend_class
